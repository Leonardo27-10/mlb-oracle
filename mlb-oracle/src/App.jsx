...
// ─── PARLAY BUILDER ───────────────────────────────────────────────────────────
const buildParlays = (predictions, count = 10) => {
  if (predictions.length < 3) return [];

  // ── Paso 1: Por cada juego elegir LA MEJOR selección según el modelo ──
  // Regla de oro: MÁXIMO 1 selección por juego, sin excepciones.
  // Esto evita: COL ML + AZ ML (mismo juego), OVER + UNDER (mismo juego), etc.
  ...
  const gameOptions = predictions.map((g) => {
    ...
    const favProb = Math.max(hProb, aProb);
    const favAbbr = hProb >= aProb ? g.home.abbr : g.away.abbr;
    const dogAbbr = hProb < aProb ? g.home.abbr : g.away.abbr;
    const dogProb = Math.min(hProb, aProb);

    const candidates = [
      // ML favorito
      { game: `${g.away.abbr} @ ${g.home.abbr}`, pick: favAbbr, type: "ML", prob: favProb, odds: toAmerican(favProb), confidence: g.confidence, value: favProb * g.confidence },
      // ML underdog (valor si prob > 40%)
      { game: `${g.away.abbr} @ ${g.home.abbr}`, pick: dogAbbr, type: "ML", prob: dogProb, odds: toAmerican(dogProb), confidence: 100 - g.confidence, value: dogProb * (100 - g.confidence) * 0.8 },
      // Over
      { game: `${g.away.abbr} @ ${g.home.abbr}`, pick: `OVER ${ou.toFixed(1)}`, type: "O/U", prob: overProb, odds: toAmerican(overProb), confidence: Math.round(overProb * 100), value: overProb * 52 },
      // Under
      { game: `${g.away.abbr} @ ${g.home.abbr}`, pick: `UNDER ${ou.toFixed(1)}`, type: "O/U", prob: underProb, odds: toAmerican(underProb), confidence: Math.round(underProb * 100), value: underProb * 50 },
    ];
    // Run line solo si hay favorito claro (>60%)
    if (favProb > 0.60) {
      candidates.push({ game: `${g.away.abbr} @ ${g.home.abbr}`, pick: `${favAbbr} -1.5`, type: "RL", prob: favProb * 0.72, odds: toAmerican(favProb * 0.72), confidence: Math.round(favProb * 72), value: favProb * 0.72 * 65 });
    }
    return { gameKey: `${g.away.abbr}@${g.home.abbr}`, candidates };
  });

  // ── Paso 2: Estrategias — cada una elige 1 candidato por juego ──
  const strategies = [
    {
      name: "🔥 Alta Confianza",
      color: "#e85d04",
      // Solo juegos con confianza >= 58, elige el candidato de mayor confianza
      pickFrom: (cands) => cands.filter(c => c.confidence >= 58).sort((a,b) => b.confidence - a.confidence)[0],
      minGames: 4,
    },
    {
      name: "💎 Valor ML Puro",
      color: "#f59e0b",
      // Solo ML, favorito de cada juego
      pickFrom: (cands) => cands.filter(c => c.type === "ML").sort((a,b) => b.prob - a.prob)[0],
      minGames: 3,
    },
    {
      name: "🎯 Mixto Inteligente",
      color: "#00e5a0",
      // El candidato de mayor valor compuesto (prob * confianza)
      pickFrom: (cands) => [...cands].sort((a,b) => b.value - a.value)[0],
      minGames: 3,
    },
    {
      name: "⚡ Run Lines",
      color: "#4a8ab5",
      // Solo RL, si no hay RL en ese juego toma el ML favorito
      pickFrom: (cands) => cands.find(c => c.type === "RL") || cands.filter(c => c.type === "ML").sort((a,b) => b.prob - a.prob)[0],
      minGames: 3,
    },
    {
      name: "📊 Totales O/U",
      color: "#a855f7",
      // Mejor over/under del juego
      pickFrom: (cands) => cands.filter(c => c.type === "O/U").sort((a,b) => b.prob - a.prob)[0],
      minGames: 3,
    },
    {
      name: "🔥 Favoritos Claros",
      color: "#ec4899",
      // ML con prob > 55%
      pickFrom: (cands) => cands.filter(c => c.type === "ML" && c.prob > 0.55).sort((a,b) => b.prob - a.prob)[0],
      minGames: 3,
    },
    {
      name: "💎 ML + RL Combinado",
      color: "#06b6d4",
      // Alterna ML y RL por juego según mayor valor
      pickFrom: (cands) => cands.filter(c => c.type !== "O/U").sort((a,b) => b.value - a.value)[0],
      minGames: 3,
    },
    {
      name: "🎯 Underdog Value",
      color: "#84cc16",
      // Selección con las mejores momios positivas (+)
      pickFrom: (cands) => [...cands].filter(c => c.prob < 0.50).sort((a,b) => b.prob - a.prob)[0]
                 || [...cands].sort((a,b) => b.value - a.value)[0],
      minGames: 3,
    },
    {
      name: "⚡ Modelo Compuesto",
      color: "#f97316",
      // El de mayor score de valor ponderado
      pickFrom: (cands) => [...cands].sort((a,b) => (b.prob * b.confidence) - (a.prob * a.confidence))[0],
      minGames: 3,
    },
    {
      name: "📊 Máximo Pago",
      color: "#6366f1",
      // Maximiza el pago potencial (menor prob pero válida > 35%)
      pickFrom: (cands) => [...cands].filter(c => c.prob > 0.35).sort((a,b) => a.prob - b.prob)[0],
      minGames: 3,
    },
  ];
  ...
  return strategies.map((strat, i) => {
    const picks = [];
    for (const { candidates } of gameOptions) {
      const pick = strat.pickFrom(candidates);
      if (pick) picks.push(pick);
    }

    // Filtrar picks válidos (prob > 0)
    const validPicks = picks.filter(p => p && p.prob > 0);

    // Si no hay suficientes picks, skip
    if (validPicks.length < strat.minGames) return null;

    const combinedProb = validPicks.reduce((acc, p) => acc * p.prob, 1);
    const decimalPayout = validPicks.reduce((acc, p) => acc * probToDecimal(p.prob), 1);
      ...
      <div onClick={() => setOpen(o=>!o)} style={{ padding:"16px 18px", cursor:"pointer", display:"flex", justifyContent:"space-between", alignItems:"center", gap:"12px" }}>
        <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
          ...
          <div>
            ...
            <div style={{ fontSize:"10px", color:"#3a5068" }}>{parlay.picks.length} selecciones · 1 pick/juego</div>
          </div>
        </div>
...
export default function App() {
  ...
  const TABS = [
    ...
    { id:"parlays", label:"🎯 Parleys", count:parlays.length },
  ];
          ...
          <div style={{ animation:"fadeUp 0.35s ease" }}>
            ...
            {tab==="parlays" && (
              <>
                ...
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:"16px", flexWrap:"wrap", gap:"8px" }}>
                  <span style={{ fontSize:"11px", color:"#3a5068", textTransform:"uppercase", letterSpacing:"0.1em" }}>10 parleys · 1 selección por juego · ML + RL + O/U</span>
                </div>
                {parlays.length===0
